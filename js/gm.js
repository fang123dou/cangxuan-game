...
const GM = (() => {
  ...
  const SITUATIONS = [
       ...
       双双承接——写打熬余韵、进境被瞧见、瓶颈契机；build 时即焚，不空转不滞留。 */
    {
      ...
      build() {
        ...
        if (m.kind === "武技磨砺" && (S.skills["乱拳"] || 0) >= (TECH_CAPS["乱拳"] || 100)) return { // 乱拳已圆：老卒改授化境之理
          scene: `老槐树下，那瘸腿老卒又「恰好」路过。他看了一眼你收拳的架势，眯起眼：「拳圆了。接下来不是练拳，是练拳的人。」他丢给你半册手抄的打熬图谱。`,
          choices: [
            { label: "照图谱打熬筋骨", hint: "力量 +0.10，体质 +0.05，体力 -2。", fx: { attr: { str: 0.1, con: 0.05 }, sta: -2 } },
            { label: "与老卒坐而论拳", hint: "道心 +0.4，缘分 +3。", fx: { dao: 0.4, npc: { "瘸腿老卒": 3 } } },
          ],
        };
  ...
  SITUATIONS.push({
    ...
    build() {
      ...
      const xz = S.gear && S.gear.xianqi; // 仙器 × 终 Boss 联动：持器者，终战有回响
      const xzLine = xz === "zhanjieDao" ? " 你背后的斩界刀先动了——三万年前的崩刃，认得这只握刀的手。"
        : xz === "liangtianchi" ? " 怀里的量天尺烫得像块炭——量过你的尺子，此刻正在量它。"
        : xz === "guiXuAnchor" ? " 归墟锚在你背后低鸣，裂缝的黑暗第一次退了半寸。"
        : xz === "wangchuanZhan" ? " 忘川盏在行囊里轻轻一晃——它照不出这具化身，因为化身也算「被写下的因果」。"
        : "";
      return {
        scene: "进食断了。归墟深处刚静下来，天——真的「下来」了。云海向两边翻开，一道和你等高、和你等形、连面板上的字迹都分毫不差的人影立在裂缝之前。它开口，是你的声音：「刀养得不错。可刀柄，从来在握刀的人手里。」" + xzLine + " 天道化身——你要不要，先试试斩这只握刀的手？",
        choices: [
